...
private const val EXAMPLE = "5 1 9 5\n" +
        "7 5 3\n" +
        "2 4 6 8"

private const val EXAMPLE2 = "5 9 2 8\n" +
        "9 4 7 3\n" +
        "3 8 6 5\n" +
        "\n"

private const val X = "157\t564\t120\t495\t194\t520\t510\t618\t244\t443\t471\t473\t612\t149\t506\t138\n" +
...
private const val X2 = "157\t564\t120\t495\t194\t520\t510\t618\t244\t443\t471\t473\t612\t149\t506\t138\n" +
        "1469\t670\t47\t604\t1500\t238\t1304\t1426\t54\t749\t1218\t1409\t60\t51\t1436\t598\n" +
        "578\t184\t2760\t3057\t994\t167\t2149\t191\t2913\t2404\t213\t1025\t1815\t588\t2421\t3138\n" +
        "935\t850\t726\t155\t178\t170\t275\t791\t1028\t75\t781\t138\t176\t621\t773\t688\n" +
        "212\t977\t297\t645\t229\t194\t207\t640\t804\t509\t833\t726\t197\t825\t242\t743\n" +
...
fun main(args: Array<String>) {
    println(test1(EXAMPLE))
    println()
    println(test1(X))
    println()
    println(test2(EXAMPLE2))
    println(test2(X2))
}

private fun test1(source: String): Int {
    var result = 0
    val lines = source.split("\n".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    for (line in lines) {
        val stat =
            Arrays.stream(Arrays.stream(line.split("\\D".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()).mapToInt {
                Integer.parseInt(
                    it
                )
            }.toArray()
        ...
        print("min: " + stat.min)
        println()
        result += stat.max - stat.min
    }
    return result
}

private fun test2(source: String): Int {
    var result = 0
    val lines = source.split("\n".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray()
    for (line in lines) {
        val numberArray = Arrays.stream(line.split("\\D".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
            .mapToInt { Integer.parseInt(it) }.toArray()
        println(Arrays.toString(numberArray))
        for (i in numberArray.indices) {
            for (j in numberArray.indices) {
                if (i != j && numberArray[i] % numberArray[j] == 0) {
                    result += numberArray[i] / numberArray[j]
                }
            }
        }
    }
